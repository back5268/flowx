...
const CustomNode = ({ data, isConnectable }: NodeProps<NodeData>) => {
    ...
    return (
        <div className={`bg-[#1E293B] p-4 rounedd border shadow-md min-w-[180px] relative cursor-pointer transition-colors rounded-xl
        ${hasError ? 'border-red-500 ring-2 ring-red-600' : isRunning ? 'border-green-400 ring-green-500 animate-pulse'
            ...
            {["left", "right", "top", "bottom"].map(pos => (
                <Handle key={pos} type={pos === "left" || pos === "top" ? "target" : "source"}
                    position={
                        ReactFlowPosition[(pos.charAt(0).toUpperCase() + pos.slice(1)) as keyof typeof ReactFlowPosition]
                    } id={pos} isConnectable={isConnectable} className="w-2 h-2 bg-green-400" />
            ))}
        </div>
    )
}
