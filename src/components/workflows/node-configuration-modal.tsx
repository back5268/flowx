...
const OAUTH_PROVIDERS = ["stripe", "gmail", "slack", "discord", "notion", "sheets"]
const API_KEY_PROVIDERS = ["openai", "gemini", "claude"]
const CORE_PROVIDERS = ["webhook-trigger", "schedule-trigger", "http-request", "send-notification", "ai-generate", "delay"]
...
const isOAuthProvider = (p: string) => OAUTH_PROVIDERS.includes(p.toLowerCase())
const isCoreProvider = (p: string) => CORE_PROVIDERS.includes(p.toLowerCase())
const isApiKeyProvider = (p: string) => API_KEY_PROVIDERS.includes(p.toLowerCase())

const NodeConfigurationModal = ({
    isOpen, onClose, nodeData, onConfigured
                        ...
                        <Field label="Webhook URL">
                            <div className="p-3 bg-[#0B0F14] border border-gray-600 rounded-md">
                                <code className="text-sm text-green-400">
                                    https://api.flowx.com/webhook/your-webhook-id
                                </code>
                            </div>
                ...
                return (
                    <div className="space-y-3">
                        <Field label="Schedule Type">
                            <select className={selectCls} value={coreConfig.scheduleType || "interval"}
                                onChange={e => setCoreConfig({ ...coreConfig, scheduleType: e.target.value })}>
                        ...
                        {coreConfig.scheduleType === "cron" ? (
                            <Field label="Cron Expression">
                                <input type="text" value={coreConfig.cronExpression || ""} placeholder="0 */5 * * * *" className={inputCls}
                                    onChange={e => setCoreConfig({ ...coreConfig, cronExpression: e.target.value })} />
                            </Field>
                        ) : <Field label="Interval (minutes)">
                            <input type="number" min={1} value={coreConfig.interval || ""} placeholder="5" className={inputCls}
                ...
                return (
                    <div className="space-y-3">
                        <Field label="HTTP Method">
                            <select className={selectCls} value={coreConfig.method || "GET"}
                        ...
                        <Field label="URL">
                            <input type="url" value={coreConfig.url || ""} placeholder="https://api.example/endpoint" className={inputCls}
                                onChange={e => setCoreConfig({ ...coreConfig, url: e.target.value })} />
                        </Field>
                    </div>
                )
                ...
                return (
                    <div className="space-y-3">
                        <Field label="Notification Channel">
                            <select className={selectCls} value={coreConfig.channel || "email"}
                                onChange={e => setCoreConfig({ ...coreConfig, channel: e.target.value })}>
                        ...
                        <Field label="Message Template">
                            <textarea value={coreConfig.message || ""} placeholder="Your notification message..." className={`${inputCls} h-20`}
                ...
                return (
                    <div className="space-x-3">
                        <Field label="AI Provider">
                            <select className={selectCls} value={coreConfig.provider || "openai"}
                        ...
                        <Field label="Prompt Template">
                            <textarea value={coreConfig.prompt || ""} placeholder="Your AI prompt template..." className={`${inputCls} h-20`}
                ...
                return (
                    <div className="space-y-3">
                        <Field label="Condition Expression">
                            <input type="text" value={coreConfig.expression || ""} placeholder="prev.status === 'paid'" className={inputCls}
                                onChange={e => setCoreConfig({ ...coreConfig, expression: e.target.value })} />
                            <p className="text-xs text-gray-400 mt-1">
                                Use JavaScript-like expression to evaluate conditions.
                            </p>
                        </Field>
                    </div>
                )
                    ...
                    <div className="space-y-3">
                        <Field label="Delay Duration">
                            <div className="flex space-x-2">
                                <input type="number" min={1} value={coreConfig.duration || ""} placeholder="5" className={`${inputCls} flex-1`}
                                    onChange={e => setCoreConfig({ ...coreConfig, duration: e.target.value })} />
                                <select className={selectCls} value={coreConfig.unit || "seconds"}
                                    onChange={e => setCoreConfig({ ...coreConfig, unit: e.target.value })}>
                ...
                return (
                    <div className="p-3 bg-blue-900/20 border border-blue-800 rounded-md">
                        <p className="text-sm text-blue-400">
                            This {displayName} node is ready to use. No additional configuration required.
                        </p>
                    </div>
                )
        }
    ...
    return <div className="fixed inset-0 backdrop-blur-sm bg-black/30 flex items-center justify-center z-50">
        <div className="bg-[#1E293B] rounded-lg p-6 w-full max-w-lg mx-4 border border-green-700 max-h-[90vh] overflow-y-auto">
                        ...
                        {isOAuth && (
                            <div className="space-y-3">
                                    ...
                                    {connections?.length > 0 ? (
                                        connections.map((connection, index) => (
                                            <div key={index} className={`p-3 rounded-md border cursor-pointer transition-colors 
                                                ${selectedConnection === connection.id ? "border-green-400 bg-green-900/20" : "border-gray-700 bg-[#0B0F14]"}`}
                                ...
                                <div className="flex justify-between mt-2">
                                    <Button variant="outline" className="border-[#334155] text-gray-300 hover:bg-[#1E293B] hover:text-white"
                            ...
                            <div className="space-y-3">
                                ...
                                {connections?.length > 0 && (
                                    <Field label="Saved Keys">
                                        <select className={selectCls} value={selectedConnection || ""} onChange={e => setSelectedConnection(e.target.value)}>
                                ...
                                <Field label="API Key">
                                    <input type="password" value={apiKey} placeholder="sk-..." className={inputCls}
            ...
            <div className="mt-6 flex justify-end gap-2">
                <Button variant="outline" className="border-[#334155] text-gray-300 hover:bg-[#1E293B] hover:text-white" onClick={onClose}>
                    Cancel
                </Button>
                <Button disabled={!!saveDisabled} className="bg-green-500 hover:bg-green-600 text-black font-medium" onClick={handleSave}>
                    Save Configuration
                </Button>
            </div>
        </div>
