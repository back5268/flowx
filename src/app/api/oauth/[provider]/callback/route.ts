...
const PROVIDERS_CONFIGS = {
    ...
    discord: {
        ...
        headers: (token: string) => ({ Authorization: `Bearer ${token}` }),
        extractAccountName: (data: any) => data.username ? `${data.username}#${data.discriminator || "0000"}` : "Discord User",
        extractUserInfo: (data: any) => data,
    },
    notion: {
        endpoint: "https://api.notion.com/v1/users/me",
        headers: (token: string) => ({ Authorization: `Bearer ${token}`, "Notion-Version": "2022-06-28", "Content-Type": "application/json" }),
        extractAccountName: (data: any) => data.person?.email ?? data.name ?? "Notion user",
        extractUserInfo: (data: any) => data,
    },
    stripe: {
        isSpecial: true,
        extractAccountName: (tokens: any) => tokens.stripe_user_id ?? "Stripe Account",
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ provider: string }> }) {
    ...
    if (!cfg) return NextResponse.json({ error: "Unknow provider" }, { status: 404 })
    ...
    try {
        ...
        console.log("Recevied token: ", tokens);
        if (!tokens.access_token) {
            console.log("No access token in the response: ", tokens);
            return NextResponse.json({ error: "No access token in the response" }, { status: 400 })
        }
        ...
        if (existingConnection.length > 0) {
            ...
            if (encryptedRefreshToken) {
                updateData.refresh_token_enc = Buffer.from(encryptedRefreshToken, "utf8")
            }

            await db.update(connections).set(updateData).where(eq(connections.id, existingConnection[0].id))
        } else {
        ...
        return `${redirectUrl}${redirectUrl.includes("?") ? "&" : "?"}connected=${provider}`
    } catch (error) {
...
async function deriveAccountName(provider: string, tokens: any) {
    try {
        const config: any = PROVIDERS_CONFIGS[provider as keyof typeof PROVIDERS_CONFIGS]
        if (!config) {
        ...
        if (!config.isSpecial) {
            const endpoint = config.useTokenInUrl ? `${config.endpoint}?token=${tokens.access_token}` : config.endpoint
            const response = await fetch(endpoint, {
                headers: config.headers(tokens.access_token)
            })
            if (!response.ok) {
                console.log(`Failed to fetch userinfo from ${provider}: `, await response.text);
                return { accountName: "Connected Account", userInfo: null }
            }
        ...
        console.log(`Error deriving acccount name for ${provider}: `, error);
    }
