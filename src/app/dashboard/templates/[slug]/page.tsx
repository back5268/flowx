...
import { nodeTypes } from "@/components/workflows/custom-node";
import { buildInitialFlow, EDGE_STYLE, reindexStepNumbers, TOOL_NODES } from "@/components/workflows/flow-utils";
import NodeConfigurationModal from "@/components/workflows/node-configuration-modal";
...
const findProviderMeta = (label: string) => {
    const category = TOOL_NODES?.find(cat => cat.items?.some(i => i.name === label))
    const item = category?.items?.find(i => i.name === label)
    return { category: category?.category ?? "custom", item, providerId: item?.id ?? null }
}
...
const Page = () => {
        ...
        setNodes(pre => (
            pre.map(n => {
                ...
                if (sn === stepNumber) {
                    return {
                        ...n, data: {
                            ...(n.data as any), isConfigured: true, config: config ?? (n.data as any)?.config ?? null
                        }
                    }
                }
                return n
    ...
    useEffect(() => {
        if (selectedNode) {
            ...
            if (updated && Boolean((updated.data as any)?.isConfigured) !== Boolean((selectedNode.data as any)?.isConfigured)) {
    ...
    useEffect(() => {
        ...
        const { nodes: intialNodes, edges: intialEdges } = buildInitialFlow(foundTemplate, configuredSteps)
        setNodes(reindexStepNumbers(intialNodes, intialEdges))
    ...
    return (
        <div className="flex h-full">
            <div className="flex-1 p-6 flex flex-col h-full">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        {[
                            { icon: Play, text: "Test", variant: "outline", onClick: handleTestWorkflow },
                            { icon: Save, text: "Save", variant: "default", onClick: handleSaveWorkflow },
                        ].map(({ icon: Icon, text, variant, onClick }) => (
                            <Button key={text} variant={variant as any}
                                className={variant === "outline" ? "border-[#334155] text-gray-300 hover:bg-[#1E293B] hover:text-white"
                                    : "bg-green-500 hover:bg-green-600 text-black font-medium"} onClick={onClick} >
                    ...
                    <CardContent className="p-0 h-full">
                        <div ref={reactFlowWrapper} className="h-full w-full">
                            <ReactFlow nodes={nodes} edges={edges} onNodesChange={handleNodesChange} onEdgesChange={onEdgesChange}
                                ...
                                onNodeClick={onNodeClick} onNodeDoubleClick={onNodeDoubleClick} nodeTypes={nodeTypes} fitView attributionPosition="bottom-right" className="bg-[#0B0F14]">
            ...
            <div className="w-80 border-l border-[#1E293B] p-4 overflow-y-auto">
                <Tabs defaultValue="nodes">
                    <TabsList className="w-full bg-[#0B0f14] mb-4">
                        <TabsTrigger value="nodes" className="flex-1 data-[state=active]:bg-[#1E293B] data-[state=active]:text-green-400" >
                            Nodes
                        </TabsTrigger>
                        <TabsTrigger value="properties" className="flex-1 data-[state=active]:bg-[#1E293B] data-[state=active]:text-green-400" >
                            Properties
                        </TabsTrigger>
                    </TabsList>
                    <TabsContent value="nodes" className="spacy-y-4">
                        {TOOL_NODES.map((category, index) => (
                            <div key={index}>
                                ...
                                {category.items.map((node, index) => (
                                    <div key={index} className="flex items-center p-2 rounded-md hover:bg-[#1E293B] cursor-grab transition-colors"
                                        draggable onDragStart={event => onDragStart(event, node)}>
                        ...
                        {selectedNode ? (
                            <div className="space-y-4">
                                ...
                                {(() => {
                                    ...
                                    const accountsContent = countLoading ? (
                                        ...
                                        accounts?.length > 0 ? (
                                            <ul className="space-y-2">
                                                {accounts.map((account, index) => <li key={index}>
                                                    <div className="flex items-center justify-between bg-[#1E293B] rounded-md p-2 border border-[#334155]">
            ...
            <NodeConfigurationModal onConfigured={markStepConfigured} isOpen={isModalOpen} onClose={closeModal} nodeData={modalNodeData} />
        </div>
    )
}
