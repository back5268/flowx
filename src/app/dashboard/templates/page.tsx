...
const Templates = () => {
  ...
  const filteredTemplates = mockTemplates.filter(template => {
    const matchesSearch = template.name.toLowerCase().includes(searchItem.toLowerCase()) || template.description.toLowerCase().includes(searchItem.toLowerCase())
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center space-x-4 bg-[#121826] p-4 rounded-lg border-[#1E293B]">
        <div className="relative flex-1 max-w-md">
          ...
          <Input placeholder="Search templates ..." value={searchItem} onChange={e => setSearchItem(e.target.value)}
            className="pl-10 bg-[#1E293B] border-[#334155] text-gray-200" />
        </div>

        <div className="flex items-center space-x-2">
          {categories.map((category, index) => (
            <Button key={index} variant={selectedCategory === category ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(category)}
              className={selectedCategory === category ? "bg-green-500 hover:bg-green-600 text-black" : "border-[#334155] text-gray-300 hover:bg-[#1E293B] hover:text-white"}>
      ...
      {filteredTemplates.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredTemplates?.map((template, index) => (
            <TemplateCard key={index} template={template} onUseTemplate={handleUseTemplate} onPreview={handlePreview} />
          ))}
        </div>
      ) : (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">
            Try adjusting your search terms of category filters
          </p>
        </div>
      )}
